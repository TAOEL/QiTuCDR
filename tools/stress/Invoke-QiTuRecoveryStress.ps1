...
try {
    ...
    $reportPath = Join-Path $OutputDirectory "qitucdr-recovery-stress-$timestamp.md"
    ...
    if ($running -and -not $NoBuild) {
        ...
        $tempOut = Join-Path $env:TEMP "qitucdr-hostharness-recovery-out-$timestamp"
        ...
        $process = Start-Process -FilePath $exe -ArgumentList @("--recovery-stress", $Iterations.ToString(), "--delay-ms", $DelayMs.ToString(), "--report", $reportPath) -Wait -PassThru
        if ($process.ExitCode -ne 0) {
            throw "Recovery stress failed with exit code $($process.ExitCode)."
        }
    }
    else {
        $arguments = @(
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Recovery stress failed with exit code $LASTEXITCODE."
        }
    }

    if (-not (Test-Path $reportPath)) {
        throw "Recovery stress did not create report: $reportPath"
    }

    Write-Host "Recovery stress report: $reportPath" -ForegroundColor Green
}
finally {
    Pop-Location
}
