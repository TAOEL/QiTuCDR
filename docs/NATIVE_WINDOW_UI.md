...
- 按 `ESC` 关闭当前弹层。
- 点击弹层外部后，弹层自动关闭。
- 弹层位置支持本地配置持久化：`保存窗口位置` 开启时，拖动后关闭再打开或重启预览器，会优先恢复上次位置；关闭该开关后不保存也不恢复主窗口和弹层位置。
...
- 置顶激活态需要增加可见状态。
- 设置弹层已接入本地配置：`窗口置顶` 即时控制当前窗口 `Topmost`，`保存窗口位置` 控制主窗口和标题栏弹层的位置持久化，`保存配置参数` 已预留给后续工具参数默认值。
